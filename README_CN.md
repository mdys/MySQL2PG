...
开始
 ...
 │     │   └─ 添加 DISTRIBUTED BY (pk_col1, pk_col2, ...) 子句
 ...
 │     ├─ 若 MPP 启用（Greenplum）:
 │     │   ├─ 跳过 UNIQUE INDEX 创建（分布键已确保唯一性）
 │     │   └─ 对有主键的表应用 ALTER TABLE ... SET DISTRIBUTED BY
 │     ├─ 主键、唯一索引、普通索引、全文索引 → 自动重建
 │     └─ 批量处理（max_indexes_per_batch=20）
 │
 ├─▶ [Step 6] 转换函数 (functions: true)
 │     ├─ 若 exclude_use_function_list=true → 跳过 exclude_function_list 中的函数
...
### 视图和函数排除功能
- **功能说明**：提供视图和函数的排除列表，灵活控制同步的视图和函数，属于转换选项配置
- **视图排除**：
  - `conversion.options.exclude_use_view_list: true` - 启用视图排除模式，跳过 exclude_view_list 中的视图
  - `conversion.options.exclude_view_list: [view1, view2]` - 指定要跳过的视图列表
- **函数排除**：
  - `conversion.options.exclude_use_function_list: true` - 启用函数排除模式，跳过 exclude_function_list 中的函数
  - `conversion.options.exclude_function_list: [func1, func2]` - 指定要跳过的函数列表
- **注意事项**：
  - 视图名和函数名不区分大小写，会自动转换为小写进行匹配
  - 配置使用字符串数组语法，例如 `[view1, view2]`
  - 跳过的对象会在日志中记录，并更新进度统计

### MPP 分布式数据库支持
- **功能说明**：为 MPP（大规模并行处理）分布式数据库（如 Greenplum、YugabyteDB）提供支持
- **参数配置**：
  - `conversion.mpp.enabled: true` - 启用 MPP 模式（创建 UNIQUE INDEX 和 DISTRIBUTED BY 子句）
  - `conversion.mpp.database: auto` - MPP 数据库类型：`greenplum`、`yugabyte` 或 `auto`（自动检测）
- **功能特性**：
  - **分布键选择**：自动使用主键列作为分布键
  - **DISTRIBUTED BY 子句**：表创建后添加 `ALTER TABLE schema.table SET DISTRIBUTED BY (col1, col2, ...)`
  - **UNIQUE INDEX 处理**：在 Greenplum 上跳过 UNIQUE INDEX 创建（分布键已确保唯一性）
  - **自动检测**：通过查询 PostgreSQL 版本/扩展自动检测 MPP 数据库类型
- **语法示例**：
  ```sql
  -- MySQL 主键
  ...
  ```
- **使用场景**：
  - 将 MySQL 表迁移到 Greenplum 分布式表
  - 迁移到 YugabyteDB 并配置正确的分布键
  - 确保数据在 MPP 段/节点间均匀分布

### HTML 迁移报告
- **功能说明**：从转换日志生成可视化 HTML 报告，采用深色终端风格
- **命令**：
  ```bash
  # 基本用法
  ...
  ```
- **功能特性**：
  - **单文件 HTML**：内联 CSS，无外部依赖，直接在浏览器中打开
  - **深色终端设计**：JetBrains Mono 字体，DM Sans 正文字体，霓虹色点缀（青色、蓝色、绿色、红色、琥珀色、紫色）
  - **统计卡片**：表、行、视图、索引、函数、错误数量统计
  - **性能柱状图**：各阶段耗时可视化，进度条展示
  - **表明细**：每表状态徽章（已转换、已存在、数据一致、数据不一致、空表）
  - **错误/警告区块**：去重后的错误信息和警告，关联到具体表
  - **不一致报告**：行数不匹配的表（MySQL vs PostgreSQL）
  - **进度追踪**：显示迁移完成状态（完成/进行中）
- **解析的日志模式**：
  - 表转换成功/跳过消息
  - 分页数据同步完成消息
  - 阶段汇总表格（同时写入控制台和日志文件）
...
### 13. HTML 迁移报告
- **命令**: `./mysql2pg report -l conversion.log`
- **深色终端风格** 使用 JetBrains Mono 字体和霓虹色点缀
- **单文件 HTML** 内联 CSS — 无外部依赖
- **去重机制**: 所有日志条目按表名去重
- **进度追踪**: 显示迁移完成状态（完成/进行中）
- **报告区块**: 统计卡片、性能柱状图、表明细、不一致表、警告、错误
- **控制台输出**: 阶段汇总表格和不一致表统计同时写入控制台和日志文件，供 report 命令解析

### 14. MPP 分布式数据库支持
- **Greenplum/YugabyteDB 支持**: 自动为 MPP 数据库添加 `DISTRIBUTED BY` 子句
- **智能分布键选择**: 默认使用主键列作为分布键
- **自动检测**: 自动检测 MPP 数据库类型（greenplum/yugabyte/auto）
- **UNIQUE INDEX 处理**: 在 Greenplum 上跳过 UNIQUE INDEX 创建（使用分布键确保唯一性）
- **配置方式**: 通过 `conversion.mpp.enabled: true` 启用
- **分布语法**: `ALTER TABLE schema.table SET DISTRIBUTED BY (col1, col2, ...)`
...
mysql:
  ...
  connection_params: charset=utf8mb4&parseTime=false&interpolateParams=true&readTimeout=60s&writeTimeout=60s&timeout=30s # MySQL连接参数
...
报告生成一个 **深色主题的单一 HTML 文件** 仪表板，包含：
- 统计卡片（表、行、视图、索引、函数、错误）
- 按阶段的性能柱状图
- 表明细含状态徽章和错误/警告指示器
- 数据不一致表
- 警告和错误区块
