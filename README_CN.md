...
- **视图排除**：
  ...
  - `conversion.options.exclude_view_list` - 指定要跳过的视图列表（对象列表格式）
- **函数排除**：
  - `conversion.options.exclude_use_function_list: true` - 启用函数排除模式，跳过 exclude_function_list 中的函数
  - `conversion.options.exclude_function_list` - 指定要跳过的函数列表（对象列表格式）
- **注意事项**：
  - 视图名和函数名不区分大小写，会自动转换为小写进行匹配
  - 排除列表必须使用对象列表语法，例如 `- name: view1`
  - 不要使用字符串数组（如 `[view1, view2]`）或布尔值作为这两个列表字段
...
conversion:
  ...
  options:
    ...
    exclude_view_list:                  # 要跳过的视图列表，当exclude_use_view_list为true时生效（对象列表格式）
      - name: view1
      - name: view2
    
    # 函数排除列表
    exclude_use_function_list: false    # 是否使用函数排除列表
    exclude_function_list:              # 要跳过的函数列表，当exclude_use_function_list为true时生效（对象列表格式）
      - name: func1
      - name: func2
